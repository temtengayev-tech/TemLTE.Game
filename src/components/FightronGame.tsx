...
import { fighters, type FighterInfo, type GameSetup } from '../game/fighters'
...
export function FightronGame({ setup, onExit, onCareerComplete, isTitleFight = true, titleDefense = false, careerEvent = 1 }: { setup: GameSetup; onExit: () => void; onCareerComplete?: (result: CareerFightResult) => void; isTitleFight?: boolean; titleDefense?: boolean; careerEvent?: number }) {
  ...
  const playerMaxHealth = setup.mode === 'brawl' ? 500 : playerInfo.maxHealth + growth * 10; const opponentMaxHealth = setup.mode === 'brawl' ? 500 : opponentInfo.maxHealth + growth * 10
  const playerMaxStamina = 300 + growth * 6; const opponentMaxStamina = 300 + growth * 6
  ...
  const attack = useCallback((side: Side, kind: 'special' | 'punch' | 'slide' | 'roundhouse') => {
    ...
    const attackDelay = action === 'punch' ? 420 : action === 'kick' ? 380 : action === 'roundhouse' ? 820 : 340
    ...
    setArena(current => {
      ...
      return { ...current, shot, [side]: { ...attacker, x: action === 'slide' ? clamp(attacker.x + direction * 11) : attacker.x, stamina: unlimitedStamina ? 300 : Math.max(0, attacker.stamina - staminaCost), action, actionUntil: now + (action === 'punch' ? 390 : action === 'kick' ? 390 : action === 'roundhouse' ? 820 : action === 'slide' ? 720 : 540), actionId: attacker.actionId + 1, ammo: ammo.current[side], reloadUntil: action === 'shoot' && ammo.current[side] === 0 ? now + 2000 : attacker.reloadUntil } }
    })
    ...
    window.setTimeout(() => setArena(current => {
      ...
      const attackerInfo = side === 'player' ? playerInfo : opponentInfo
      const techniqueBonus = action === 'slide' && ['islam', 'khabibi'].includes(attackerInfo.id) ? 10 : action === 'roundhouse' && ['conor', 'chalres'].includes(attackerInfo.id) ? 8 : action === 'punch' && attackerInfo.id === 'agent' ? 6 : 0
      const careerDamage = growth * 2
      const rawDamage = action === 'shoot' ? (distance <= 62 && target.y < 23 ? 20 + careerDamage : 0) : distance <= closeRange && targetHeightValid && !roundhouseFailed ? baseDamage + careerDamage + techniqueBonus : 0
      ...
      const extraStun = action === 'roundhouse' && ['conor', 'chalres'].includes(attackerInfo.id) ? 500 : 0
      const reactionUntil = action === 'roundhouse' ? now + 3000 + extraStun : action === 'slide' && rawDamage ? now + reactionTime : target.actionUntil
    ...
    }), action === 'punch' ? 165 : action === 'kick' ? 150 : action === 'roundhouse' ? 410 : action === 'slide' ? 310 : 225)
  }, [isPaused, opponentMaxHealth, opponentSpecial, playerMaxHealth, playerSpecial, showPrefight, unlimitedStamina])
...
function Fighter({ fighter, info, side, targetX, mmaAgent, winner, celebrating }: { fighter: FighterState; info: FighterInfo; side: Side; targetX: number; mmaAgent: boolean; winner: boolean; celebrating: boolean }) {
  ...
  return <div translate="no" className={`fighter fighter-${info.id} ${side}-side ${fighter.health <= 0 ? 'knocked-out' : ''} ${winner ? 'victory-pose' : celebrating ? 'standard-winner-pose' : ''} ${fighter.action === 'slide' ? 'sliding' : ''} ${reactionClass} ${reactionTier}`} style={{ left: `${fighter.x}%`, bottom: `${74 + fighter.y}px` }}>
    <div className={`fighter-facing ${!winner && mirrored ? 'mirrored' : ''}`}><div className={fighter.hitId ? 'damage-react' : ''}>
      {winner ? <div className="champion-animation" style={{ backgroundImage: `url('/assets/${info.id}-champion-frames.png')` }} aria-label={`${info.name} raising both hands with the FCB belt`} /> : celebrating ? <div className={`standard-winner-animation frame-sprite ${sheet} action-idle`} aria-label={`${info.name} raising both arms`} /> : <div className={`frame-sprite ${sheet} action-${fighter.action}`} />}
      ...
      {fighter.action === 'stagger' && <b className="stun-label" aria-label="Stunned" />}
...
function VictoryCelebration({ winner, side, method, titleFight, defending }: { winner: FighterInfo; side: Side; method: Arena['resultMethod']; titleFight: boolean; defending: boolean }) { return <div className={`victory-celebration ${side} ${titleFight ? 'title-victory' : 'standard-victory'}`}><div className="winner-callout">{titleFight ? <><small>{defending ? 'AND STILL · DEFENDING' : 'AND THE NEW UNDISPUTED'}</small><span>FCB CHAMPION OF THE WORLD</span></> : <small>WINNER BY {method === 'decision' ? 'DECISION' : 'KNOCKOUT'}</small>}<strong>{winner.name}</strong>{titleFight && <em>WINNER BY {method === 'decision' ? 'ROUNDS DECISION' : 'KNOCKOUT'}</em>}</div>{titleFight && <><div className="corner-team" aria-label={`${winner.name}'s team celebrating`}><i /><i /><i /></div><div className="celebration-confetti">{Array.from({ length: 18 }, (_, index) => <i key={index} />)}</div></>}</div> }
