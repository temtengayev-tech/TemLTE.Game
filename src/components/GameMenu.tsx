...
export function GameMenu({ onPlay, onRegister, onSignOut, userEmail }: Props) {
  ...
  const chooseMode = (next: GameMode) => {
    setMode(next); setPlayer('agent'); setOpponent(next === 'normal' ? 'officer' : 'john'); setSlot('player')
  }
  ...
  if (mode === 'solo' || mode === 'career') return <main className={`roster-screen solo-roster roster-${selected.id}`}>
    ...
    <section className="roster-stage solo-selection"><div className="selected-fighter"><div className="fighter-glow" /><i key={selected.id} className={`selection-hero-sprite frame-sprite ${selected.sheet} action-idle`} /><h2>{selected.name}</h2><p>“{nicknameFor(selected.id)}”</p></div><FighterStats key={selected.id} fighter={selected} mode={mode} /></section>
  ...
  return <main className={`roster-screen roster-${selected.id}`}>
    ...
    <section className="roster-stage">
      ...
      <FighterStats key={selected.id} fighter={selected} mode={mode} />
    </section>
...
function ModeScreen({ onChoose, onRegister, onSignOut, userEmail }: { onChoose: (mode: GameMode) => void; onRegister: () => void; onSignOut: () => unknown; userEmail: string | null }) {
  ...
  return <main className="fight-entry new-entry">
    ...
    <section><small>FCB WORLD CHAMPIONSHIP · MAIN CARD</small><h1>FIGHTRON</h1><div className="menu-title-belt"><img src="/assets/fcb-championship-belt.png" alt="FCB championship belt" /><span>FIGHTRON CHAMPIONSHIP BELT</span></div><p>Choose your division. Build the fight card. Enter the cage.</p><div className="entry-modes"><button onClick={() => setShowFormats(true)}><i>01</i><b>MMA</b><span>Choose a fight format</span></button><button onClick={() => onChoose('career')}><i>02</i><b>CAREER</b><span>Climb the ranks · Win the title</span></button><button onClick={() => onChoose('normal')}><i>03</i><b>STREET</b><span>Weapons · City arena</span></button><button onClick={() => onChoose('versus')}><i>04</i><b>LOCAL VS</b><span>Two fighters · One screen</span></button><button onClick={() => onChoose('solo')}><i>05</i><b>OPEN GYM</b><span>Solo training in the cage</span></button></div></section>
    <aside className="phone-mma-decor" aria-hidden="true"><img src="/assets/fcb-championship-belt.png" alt="" /><div><b>FCB WORLD TITLE</b><span>FIGHT NIGHT · ALMATY</span></div><strong>03 ROUNDS</strong><strong>ONE CHAMPION</strong></aside>
...
function FighterStats({ fighter, mode }: { fighter: typeof fighters[FighterId]; mode: GameMode }) {
  const special = mode === 'mma' && fighter.id === 'agent' ? 'KICK' : fighter.special.toUpperCase()
