...
export function CareerMode({ fighterId, save, onExit }: { fighterId: FighterId; save: CareerSave; onExit: () => void }) {
  ...
  const [event, setEvent] = useState(Number(saved.event ?? 1)); const [fighting, setFighting] = useState(false); const [showNews, setShowNews] = useState(false)
  ...
  useEffect(() => { const timer = window.setTimeout(() => { void updateCareerSave(save.id, fighterId, { event, opponent, results, championId, resting, contractMessage, upgrades, spent }) }, 500); return () => window.clearTimeout(timer) }, [championId, contractMessage, event, fighterId, opponent, resting, results, save.id, spent, upgrades])
  ...
  const restEvent = () => { applyCard(simulateEvent(event, [fighterId], championId)); setContractMessage(`You rested at Event ${event}. The other five fighters completed the card.`); setOpponent(null); setEvent(value => value + 1) }
  ...
  return <CareerHeadquarters fighterId={fighterId} championId={championId} opponent={opponent} event={event} cash={cash} purse={purse} contractMessage={contractMessage} standings={standings} choices={choices} upgrades={upgrades} upgradeInfo={upgradeInfo} onExit={onExit} onNews={() => setShowNews(true)} onRequest={request} onBuy={buy} onFight={() => setFighting(true)} onRest={restEvent} />
}
