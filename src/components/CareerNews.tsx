...
function localCareerNews(event: number, fighterId: FighterId, championId: FighterId, opponentId: FighterId | null, standings: Array<[FighterId, number]>): AiCareerNews {
  const rival = opponentId ?? standings.find(([id]) => id !== fighterId)?.[0] ?? championId
  const playerRank = standings.findIndex(([id]) => id === fighterId) + 1
  const rivalRank = standings.findIndex(([id]) => id === rival) + 1
  return { source: 'local', stories: [
    { category: 'CAREER WATCH', headline: `${fighters[fighterId].name} ENTERS EVENT ${event} AT RANK #${playerRank}`, body: `The contender's camp is focused on climbing the FCB ladder. A win in the next contract could change both the rankings and the title picture.` },
    { category: 'NEXT FIGHT', headline: `${fighters[fighterId].name} TARGETS ${fighters[rival].name}`, body: `The proposed matchup pairs rank #${playerRank} against rank #${rivalRank}. Both camps know the result could reshape the next FCB event card.` },
    { category: 'CHAMPIONSHIP', headline: `${fighters[championId].name} REMAINS THE FIGHTER TO CATCH`, body: `The FCB champion watches a changing field of contenders. Every completed event creates a new route toward the world-title contract.` },
    { category: 'UNCONFIRMED RUMOR', headline: `TRAINING CAMP CHANGE REPORTED BEFORE EVENT ${event}`, body: rumors[event % rumors.length] },
  ], poll: { first: fighterId, second: rival, firstPercent: Math.max(35, Math.min(70, 52 + rivalRank - playerRank)) } }
}
...
export function CareerNews({ results, standings, championId, fighterId, opponentId, restingId, event, contractMessage }: { results: CareerResult[]; standings: Array<[FighterId, number]>; championId: FighterId; fighterId: FighterId; opponentId: FighterId | null; restingId: FighterId | null; event: number; contractMessage: string }) {
  ...
  const refreshAiNews = useCallback(async () => {
    ...
    catch { setAiNews(localCareerNews(event, fighterId, championId, opponentId, standings)); setAiStatus('error') }
  ...
  return <section className="career-news">
    ...
    <div className="news-grid">
      ...
      {aiStatus === 'error' && <article className="ai-news-error"><span>LOCAL FALLBACK ACTIVE</span><h4>FCB COVERAGE IS STILL LIVE</h4><p>Gemini could not respond, so this edition was generated from the current career rankings, opponent and event. Add the server-side key to restore Gemini editions.</p></article>}
